...
def learn_new_user_preferences_abreviated_csv():
    ...
    abrev_ratings = csv_processes.ratings[csv_processes.ratings['user_id'] <= 10]
...
def learn_new_user_preferences_csv():
    new_user_id = csv_processes.ratings['user_id'].max() + 1
    user_ratings = input_user_ratings(csv_processes.anime, new_user_id)
        
    ratings = csv_processes.ratings.append(user_ratings, ignore_index=True)
