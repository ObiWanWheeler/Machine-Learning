...
class PopularityRecommender():
    ...
    def generate_recommendations(self, shows: pd.DataFrame, recommendation_count: int = 10, verbose: bool = False, items_to_ignore=[]) -> pd.DataFrame:
        overall_vote_average, member_minimum = calculate_popularity_stats(shows)
        filtered_recs = shows[shows['members'] >= member_minimum]
        filtered_recs['weighted_score'] = filtered_recs.apply(func=lambda show: weight_rating(show, member_minimum, overall_vote_average), axis=1)
