...
class ContentRecommender(Recommender):
    ...
    def calculate_user_embedding(self, user_id) -> dict:
        ...
        rating_tuple_anime_id_index = rating_df_cols.index("anime_id") # index of id column once df is numpyified
        ...
        for rating in this_users_ratings: # iterate through user's ratings
            this_shows_genres = list(
                self.shows.loc[self.shows["anime_id"] == rating[rating_tuple_anime_id_index]]["genre"])[0].split(", ") # extract genre data

            for genre in this_shows_genres:
                if genre not in embedding: # construct embedding baed on genres of the show
    ...
    def calculate_item_embeddings(self) -> dict:
        ...
        show_tuple_anime_id_index = show_df_cols.index("anime_id") # index of id column once df is numpyified
    ...
    def calculate_similarity_score(self, user_embedding, show_embedding) -> int:
        """Calculates similarity between user embedding and show embedding, both given as dictionaries of form genre: score
    ...
    def generate_recommendations(self, user_id: int, recommendation_count: int = 10, verbose: bool = False, items_to_ignore=[]) -> pd.DataFrame:
        ...
        filtered_scores = {id: score for id,
                           score in scores.items() if id not in items_to_ignore}
        ...
        top_shows = pd.DataFrame(sorted(filtered_scores.items(), key=lambda x: x[1], reverse=True)[
                                 :min(recommendation_count, len(filtered_scores))], columns=["anime_id", "relevance_score"])
