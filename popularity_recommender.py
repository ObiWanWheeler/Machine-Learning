...
from utils import calculate_popularity_stats

class PopularityRecommender:

    def give_recommendation(self, topn, df):
        overall_vote_average, member_minimum = calculate_popularity_stats(df)
        df = df.copy()[df['members'] >= member_minimum]

        df['weighted_score'] = self.df.apply(
            self.weight_rating, df, member_minimum, overall_vote_average, axis='columns')
