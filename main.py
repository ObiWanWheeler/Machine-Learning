...
if __name__ == '__main__':
    ...
    with connection.cursor() as cur:
        ...
        items_to_ignore = list(watched_ratings[watched_ratings['user_id'] == new_uid]['anime_id'])

        collab = CollabRecommender(watched_ratings, user_id_column='user_id', item_id_column='anime_id', rating_column='rating')
        recs = collab.give_recommendations(new_uid, anime_df, verbose=True, topn=20, items_to_ignore=items_to_ignore)
        ...
        cr = ContentRecommender(user_profiles, tfidf_matrix, anime_ids)
        recs = cr.give_recommendation(new_uid, 'anime_id', anime_df, verbose=True, topn=20, items_to_ignore=items_to_ignore)
        print(recs, '\n'*3)

        print(f'Hybrid recommendations for user {new_uid}: \n')
        hr = HybridRecommender(cr, collab, 'anime_id', {'content': 1.0, 'collab': 5.0})
        print(hr.give_recommendations(user_id=new_uid, items_df=anime_df, topn=20, verbose=True)[['anime_id', 'name', 'rating']])
